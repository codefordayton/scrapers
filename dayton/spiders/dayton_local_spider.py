...
class DaytonLocalSpider(Spider):
    ...
    def parse(self, response):
        ...
        links = sel.css(
            '#MainContentArea div.clearc a').xpath('@href').extract()
        return [Request(url=link, callback=self.paginate)
                for link in links if not link.startswith('#')]

    def paginate(self, response):
        sel = Selector(response)
        links = sel.xpath(
            "//div[contains(@class,'dright')]/a/ @href").extract()
        link_req_objs = [Request(url=link,
                                 callback=self.extract) for link in links]
        next_url = sel.xpath("//a[text()='Next']/@href").extract()
        if next_url:
            link_req_objs.append(
                Request(url=urlparse.urljoin(response.url, next_url[0]),
...
if __name__ == '__main__':
    ...
    for url in urls:
        ...
        print(DaytonLocalSpider.extract(DaytonLocalSpider(),
                                        response=response))
