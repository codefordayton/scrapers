...
class ReapSpider(scrapy.Spider):
        ...
        with open (glob.glob('delinquent/*.csv')[0], 'rb') as csvfile:
            ...
            for row in csvreader:
                ...
                if item['parcelid'].startswith('R72'):
                    request = scrapy.Request("http://mctreas.org/master.cfm?parid=" + item['parcelid'] + "&taxyr=2015" + "&own1=" + row[ownernamecol] + '\n', callback=self.getTaxEligibility)
    ...
    def getPaymentPlan(self, response):
        ...
        reapitems.writerow([item['parcelid'], item['parcellocation'], item['taxeligible'], item['paymentplan'], item['lastYear'], item['paymentwindow']])
